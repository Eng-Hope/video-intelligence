...
use crate::users::services::jwt_service::{extract_subject, generate_persisted_user_token, verify_token, TokenType};
...
/// 
/// return response
pub async fn login(state: Extension<Arc<AppState>>, login_request: Json<LoginRequest>)-> Result<Json<LoginResponse>, (StatusCode, Json<ApplicationError>)> {
    ...
    pub async fn generate_user_session(email: &str, pool: &PgPool) -> Result<LoginResponse, ApplicationError>{
...
/// 
    pub async fn authenticate_user(token: Option<&str>, details:  Option<LoginRequest>, pool: &PgPool)-> Result<LoginResponse, ApplicationError>{
       //if the token is present, then just return session using token
        if token.is_some(){
           ...
           Ok(generate_user_session(&extract_subject(&token)?, pool).await?)
            ...
            let details = details.unwrap_or(LoginRequest{email: "".into(), password: "".into()});
             match get_user_by_email(pool, &details.email).await{
                Ok(user) => {
                    match verify(details.password, user.password.clone().unwrap_or_default().as_str()) {
                        Err(e) => {
                            log::error!("{:?}", e);
                        ...
                        Ok(result) => {
                            match result {
                                    ...
                                    match generate_persisted_user_token(&details.email, pool).await {
                                        Ok(tokens) => {
                                            ...
                                            match user.to_response(pool).await {
                                                ...
                                                Err(error) => {
                                                    error!("{:?}", error);
                                                    Err(ApplicationError::generic("Cannot fetch user data please try again"))
                                                }
                                            }
                                        },
                                        Err(error) => {
                                ...
                                _ => Err(ApplicationError::new("Authentication Error", "Invalid Credentials."))
                            }
                        }
                    }
                },
                ...
                Err(e)=>  {
                    error!("{:?}", e);
                    Err(ApplicationError::new("Authentication Error","Invalid Credentials."))
                }
