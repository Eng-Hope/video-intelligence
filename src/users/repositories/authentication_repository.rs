...
pub async fn save_new_user_and_allocate_a_role(pool: &PgPool, user: &User)
    -> Result<UserResponse, ApplicationError>{

  let mut tx = pool.begin().await?;

    let saved_user =  sqlx::query_as!(User, "insert into users (id, name, email, is_enabled, is_account_non_expired,
