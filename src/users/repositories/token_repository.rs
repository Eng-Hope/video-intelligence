...
pub async fn persist_refresh_and_access_tokens(pool: &PgPool, tokens: &UserTokenResponse, email: &str)-> Result<UserTokenResponse, ApplicationError> {
    let mut tx = pool.begin().await?;
    let saved_user = get_user_by_email(pool, email).await?;
    
    //persist access token
    let access = sqlx::query_as!(Token, "insert into token(is_expired, is_revoked, id, user_id, token)
   ...
   let refresh = sqlx::query_as!(Token, "insert into token(is_expired, is_revoked, id, user_id, token)
        values ($1, $2, $3, $4, $5) returning *",
        Some(false),
        Some(false),
        Uuid::new_v4(),
